...
import  SummaryStatisticsService from '../services/summaryStatisticsService.js';
...
function RaisePurchaseOrder() {
  ...
  const [summaryStatisticsData, setSummaryStatisticsData] = useState([]);
  ...
  // 2. Fetch PO Temp
    ...
    async function fetchSummaryStatistics () {
      ...
      // API Call
      try {
          const service = new SummaryStatisticsService();
          const data = await service.getSummaryStatistics(); 
          setSummaryStatisticsData(data);

        } catch (error) {
          console.error('Failed to fetch summary statistics data:', error);
        }
      }
    ...
    async function addPOTemp() { 
      ...
      const poTemp = {
        raisedBy: localStorage.getItem('email'),
        company: company,
        quantity: quantity,
        item: item,
        unit: unit,
          ...
          <div className="statistics-container">
            ...
            <div className="statistics-table-scroll">
              <table className="statistics-table">
                  ...
                  <tbody>
                   {summaryStatisticsData.map((row, index) => (
                    <tr key={index}>
                      <td>{row[0]}</td>
                      <td>{row[1]}</td>
                      <td>{row[2]}</td>
